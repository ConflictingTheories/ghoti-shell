...
if(NOT ghoti_IN_TREE_BUILD)
    ...
    add_dependencies(ghoti_tests tests_dir)
endif()
...
# Lastly put ghoti_test_helper there too.
...
foreach(TESTTYPE test serial_test)
  add_custom_target(${TESTTYPE}_low_level
    COMMAND env XDG_DATA_HOME=test/data XDG_CONFIG_HOME=test/home ./ghoti_tests
    WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
    DEPENDS ghoti_tests
    USES_TERMINAL)

  add_custom_target(${TESTTYPE}_ghotiscript
                    COMMAND cd tests && ${TEST_ROOT_DIR}/bin/ghoti test.ghoti
                    DEPENDS test_prep
                    USES_TERMINAL)

  add_custom_target(${TESTTYPE}_interactive
      COMMAND cd tests && ${TEST_ROOT_DIR}/bin/ghoti interactive.ghoti
      DEPENDS test_prep
      USES_TERMINAL)
endforeach(TESTTYPE)
...
add_dependencies(serial_test_ghotiscript serial_test_low_level)
add_dependencies(serial_test_interactive serial_test_ghotiscript)


add_custom_target(serial_test_high_level
                  DEPENDS serial_test_interactive serial_test_ghotiscript)

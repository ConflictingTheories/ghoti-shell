...
- ``-U`` or ``--universal`` causes the specified shell variable to be given a universal scope. If this option is supplied, the variable will be shared between all the current user's ghoti instances on the current computer, and will be preserved across restarts of the shell.
...
::
    ...
    # Does the same thing as the previous two commands the way it would be done pre-ghoti 3.0.
    set foo hi
    set foo $foo there
    ...
    # Like other shells, ghoti 3.1 supports this syntax for passing a variable to just one command:
    # Run ghoti with a temporary home directory.
    HOME=(mktemp -d) ghoti

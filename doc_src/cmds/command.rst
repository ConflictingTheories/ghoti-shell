...
- ``-q`` or ``--query``, silences the output and prints nothing, setting only the exit status. Implies ``--search``. For compatibility with old ghoti versions this is also ``--quiet`` (but this is deprecated).
...
``command ls`` causes ghoti to execute the ``ls`` program, even if an ``ls`` function exists.

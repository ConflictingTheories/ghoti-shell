...
static void test_tokenizer() {
    ...
    auto saved_flags = ghoti_features();
    mutable_ghoti_features().set(features_t::stderr_nocaret, false);
    if (get_redir_mode(L"^") != redirection_mode_t::overwrite)
        err(L"redirection_type_for_string failed on line %ld", (long)__LINE__);
    mutable_ghoti_features().set(features_t::stderr_nocaret, true);
    if (get_redir_mode(L"^") != none())
        err(L"redirection_type_for_string failed on line %ld", (long)__LINE__);
    mutable_ghoti_features() = saved_flags;
}
...
static void test_abbreviations() {
    ...
    for (const auto &kv : abbreviations) {
        int ret = vars.set_one(L"_ghoti_abbr_" + kv.first, ENV_LOCAL, kv.second);
...
static void test_complete() {
    ...
    int ret = pvars.set_one(L"_ghoti_abbr_testabbrsonetwothreezero", ENV_LOCAL, L"expansion");
...
static void test_highlighting() {
    ...
    if (system("mkdir -p test/ghoti_highlight_test/")) err(L"mkdir failed");
    if (system("touch test/ghoti_highlight_test/foo")) err(L"touch failed");
    if (system("touch test/ghoti_highlight_test/bar")) err(L"touch failed");
    ...
    highlight_tests.push_back({{L"echo", highlight_role_t::command},
                               {L"test/ghoti_highlight_test/foo", param_valid_path},
    ...
    highlight_tests.push_back({
        ...
        {L"test/ghoti_highlight_test", param_valid_path},
    });

    highlight_tests.push_back({
        {L"cd", highlight_role_t::command},
        {L"test/ghoti_highlight_test/foo", highlight_role_t::error},
    });
...
static void test_string() {
    ...
    const struct string_test {
        ...
        {{L"string", L"join", L".", L"ghotishell", L"com", 0}, STATUS_CMD_OK, L"ghotishell.com\n"},
        ...
        {{L"string", L"split", L"-m0", L"/", L"/usr/local/bin/ghoti", 0},
         STATUS_CMD_ERROR,
         L"/usr/local/bin/ghoti\n"},
        {{L"string", L"split", L"-m2", L":", L"a:b:c:d", L"e:f:g:h", 0},
         STATUS_CMD_OK,
         L"a\nb\nc:d\ne\nf\ng:h\n"},
        {{L"string", L"split", L"-m1", L"-r", L"/", L"/usr/local/bin/ghoti", 0},
         STATUS_CMD_OK,
         L"/usr/local/bin\nghoti\n"},
    ...
    const auto saved_flags = ghoti_features();
    ...
    mutable_ghoti_features().set(features_t::qmark_noglob, true);
    ...
    const struct string_test qmark_glob_tests[] = {
        ...
        {{L"string", L"match", L"?*", L"ab", 0}, STATUS_CMD_OK, L"ab\n"},
        {{L"string", L"match", L"a*\\?", L"abc?", 0}, STATUS_CMD_OK, L"abc?\n"}};
    mutable_ghoti_features().set(features_t::qmark_noglob, false);

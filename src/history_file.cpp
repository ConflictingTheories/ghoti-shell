...
static size_t offset_of_next_item_ghoti_2_0(const history_file_contents_t &contents,
                                           size_t *inout_cursor, time_t cutoff_timestamp);
static size_t offset_of_next_item_ghoti_1_x(const char *begin, size_t mmap_length,
                                           size_t *inout_cursor);
...
static maybe_t<history_file_type_t> infer_file_type(const void *data, size_t len) {
    ...
    if (len > 0) {  // old ghoti started with a #
        if (static_cast<const char *>(data)[0] == '#') {
            result = history_type_ghoti_1_x;
        } else {  // assume new ghoti
            result = history_type_ghoti_2_0;
        }
    }
    return result;
}
...
maybe_t<size_t> history_file_contents_t::offset_of_next_item(size_t *cursor, time_t cutoff) const {
    ...
    switch (this->type()) {
        case history_type_ghoti_2_0:
            offset = offset_of_next_item_ghoti_2_0(*this, cursor, cutoff);
            break;
        case history_type_ghoti_1_x:
            offset = offset_of_next_item_ghoti_1_x(this->begin(), this->length(), cursor);
            break;
    }
    if (offset == size_t(-1)) {
        return none();

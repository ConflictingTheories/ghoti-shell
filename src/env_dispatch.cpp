...
/// Update the value of g_guessed_ghoti_emoji_width
static void guess_emoji_width(const environment_t &vars) {
    if (auto width_str = vars.get(L"ghoti_emoji_width")) {
        ...
        g_ghoti_emoji_width = std::max(0, new_width);
    ...
    if (term == L"Apple_Terminal" && version >= 400) {
        ...
        g_guessed_ghoti_emoji_width = 1;
        FLOGF(term_support, "default emoji width: 1");
    } else {
        // Default to whatever system wcwidth says to U+1F603,
        // but only if it's at least 1.
        int w = wcwidth(L'😃');
        g_guessed_ghoti_emoji_width = w > 0 ? w : 1;
        FLOGF(term_support, "default emoji width: %d", g_guessed_ghoti_emoji_width);
    }
}
...
void env_dispatch_var_change(const wcstring &key, env_stack_t &vars) {
    ...
    // Eww.
    if (string_prefixes_string(L"ghoti_color_", key)) {
...
static void handle_ghoti_use_posix_spawn_change(const environment_t &vars) {
    ...
    auto use_posix_spawn = vars.get(L"ghoti_use_posix_spawn");
    g_use_posix_spawn =
...
static void update_ghoti_color_support(const environment_t &vars) {
    ...
    if (auto ghoti_term256 = vars.get(L"ghoti_term256")) {
        ...
        if (term_program == L"Apple_Terminal") {
            ...
            if (tpv && ghoti_wcstod(tpv->as_string().c_str(), nullptr) > 299) {
    ...
    if (auto ghoti_term24bit = vars.get(L"ghoti_term24bit")) {
        ...
        if (vars.get(L"STY") || string_prefixes_string(L"eterm", term)) {
            ...
            if (auto ct = vars.get(L"COLORTERM")) {
                ...
                if (ghoti_wcstod(vte->as_string().c_str(), nullptr) > 3600) {

...
bool check_cancel_from_ghoti_signal() {
    switch (s_exit_state) {
...
static void reader_interactive_init(parser_t &parser) {
    ...
    // It shouldn't be necessary to place ghoti in its own process group and force control
    // of the terminal, but that works around ghoti being started with an invalid pgroup,
    ...
    // For compatibility with ghoti 2.0's $_, now replaced with `status current-command`
...
static int read_i(parser_t &parser) {
    ...
    while (!check_exit_loop_maybe_warning(data.get())) {
        ...
        if (tmp && !tmp->empty()) {
            ...
            event_fire_generic(parser, L"ghoti_preexec", &argv);
            ...
            event_fire_generic(parser, L"ghoti_postexec", &argv);
    ...
    switch (c) {
        ...
        case rl::execute: {
            ...
            if (command_test_result == 0) {
                ...
                event_fire_generic(parser(), L"ghoti_posterror", &argv);
...
maybe_t<wcstring> reader_data_t::readline(int nchars_or_0) {
    ...
    s_reset_abandoning_line(&screen, termsize_last().width);
    event_fire_generic(parser(), L"ghoti_prompt");
    exec_prompt();

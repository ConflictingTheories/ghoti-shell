...
static void source_config_in_directory(parser_t &parser, const wcstring &dir) {
    ...
    const wcstring escaped_pathname = escaped_dir + L"/config.ghoti";
    ...
    set_is_within_ghoti_initialization(true);
    parser.eval(cmd, io_chain_t());
    set_is_within_ghoti_initialization(false);
}
...
int main(int argc, char **argv) {
    ...
    if (auto features_var = env_stack_t::globals().get(L"ghoti_features")) {
        for (const wcstring &s : features_var->as_list()) {
            mutable_ghoti_features().set_from_string(s);
        }
    }
    mutable_ghoti_features().set_from_string(opts.features);
    proc_init();
    builtin_init();
    misc_init();
    reader_init();
    ...
    event_fire_generic(parser, L"ghoti_exit", &event_args);

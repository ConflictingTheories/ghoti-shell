...
static const std::vector<electric_var_t> electric_variables{
    ...
    {L"ghoti_private_mode", electric_var_t::freadonly},
...
static bool is_read_only(const wcstring &key) {
    ...
    // Hack.
    return in_private_mode() && key == L"ghoti_history";
}
...
void env_init(const struct config_paths_t *paths /* or NULL */) {
    ...
    if (const char *shlvl_var = getenv("SHLVL")) {
        ...
        long shlvl_i = ghoti_wcstol(str2wcstring(shlvl_var).c_str(), &end);
        if (!errno && shlvl_i >= 0) {
    ...
    //     env HOME=(mktemp -d) su --preserve-environment ghoti
...
bool env_stack_t::universal_barrier() {
    ...
    // This means that changes from other ghoti processes will only be visible when the "main thread
    // runs."

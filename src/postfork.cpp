...
int child_setup_process(pid_t new_termowner, pid_t ghoti_pgrp, const job_t &job, bool is_forked,
    ...
    if (new_termowner != INVALID_PID && new_termowner != ghoti_pgrp) {
        ...
        // enter this.
        if (tcgetpgrp(STDIN_FILENO) == ghoti_pgrp) {

...
bool job_t::should_report_process_exits() const {
    ...
    // external process. Bizarrely the process exit event is for the pgroup leader which may be ghoti
    // itself.
...
int terminal_maybe_give_to_job_group(const job_group_t *jg, bool continuing_from_stopped) {
    ...
    //   1. The process's pgrp is the same as ghoti. In that case there is nothing to do.
    ...
    //   4. The tty is owned by a different process. This may come about if ghoti is running in the
    //      background with job control enabled. Do not transfer it.
    //   5. The tty is owned by ghoti. In that case we want to transfer the pgid.
...
static bool terminal_return_from_job_group(job_group_t *jg, bool restore_attrs) {
    ...
    FLOG(proc_pgroup, "ghoti reclaiming terminal after job pgid", *pgid);
    ...
    // See: https://github.com/ghoti-shell/ghoti-shell/issues/2114
    if (restore_attrs) {
...
void job_t::continue_job(parser_t &parser, bool in_foreground) {
    ...
    cleanup_t take_term_back([&] {
        if (term_transferred) {
            ...
            // Issues of interest:
            // https://github.com/ghoti-shell/ghoti-shell/issues/121
            // https://github.com/ghoti-shell/ghoti-shell/issues/2114
    ...
    if (!is_completed()) {
        ...
        if (in_foreground) {
            ...
            while (!check_cancel_from_ghoti_signal() && !is_stopped() && !is_completed()) {
...
void hup_jobs(const job_list_t &jobs) {
    ...
    for (const auto &j : jobs) {
        auto pgid = j->get_pgid();
        if (pgid && *pgid != ghoti_pgrp && !j->is_completed()) {
            if (j->is_stopped()) {
                j->signal(SIGCONT);
            }
            j->signal(SIGHUP);
...
static std::atomic<bool> s_is_within_ghoti_initialization{false};

void set_is_within_ghoti_initialization(bool flag) { s_is_within_ghoti_initialization.store(flag); }

bool is_within_ghoti_initialization() { return s_is_within_ghoti_initialization.load(); }

...
static int wcsfilecmp_leading_digits(const wchar_t **a, const wchar_t **b) {
    ...
    long a_num = ghoti_wcstol(*a, &a_end, 10);
    if (errno > 0) return 0;  // invalid number -- fallback to simple string compare
    long b_num = ghoti_wcstol(*b, &b_end, 10);
    if (errno > 0) return 0;  // invalid number -- fallback to simple string compare
...
/// the current ghoti process which results in weird situations. This is basically a best effort

...
ghoti_BIN_PATH = False  # will be set later
...
def run_ghoti_cmd(text):
    ...
    if ctype is None or re.search(r"\.utf-?8$", ctype, flags=re.I) is None:
        ...
        # ghoti makes the same assumption in config.ghoti
        env = os.environ.copy()
...
class ghotiConfigTCPServer(SocketServer.TCPServer):
    ...
    def verify_request(self, request, client_address):
        return client_address[0] in ghotiConfigTCPServer.WHITELIST


class ghotiConfigHTTPRequestHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def do_get_colors(self):
        ...
        for line in out.split("\n"):
            ...
            for match in re.finditer(r"^ghoti_color_(\S+) ?(.*)", line):
    ...
    def do_get_variables(self):
        ...
        for line in out.split("\n"):
            ...
            if len(comps) < 2:
                continue
            ghoti_var = ghotiVar(comps[0], comps[1])
    ...
    def do_get_bindings(self):
        ...
        for line in out.split("\n"):
            ...
            if command in command_to_binding:
                ...
                ghoti_binding = ghotiBinding(command, line, readable_binding)
    ...
    def do_get_prompt(self, command_to_run, prompt_function_text, extras_dict):
        ...
        prompt_demo_ansi, err = run_ghoti_cmd(command_to_run)
    ...
    def do_get_current_prompt(self):
        ...
        prompt_func, err = run_ghoti_cmd("functions ghoti_prompt")
        result = self.do_get_prompt(
            'builtin cd "' + initial_wd + '" ; false ; ghoti_prompt',
            prompt_func.strip(),
            {"name": "Current"},
        )
        return result
    ...
    def do_get_sample_prompts_list(self):
        paths = glob.iglob("sample_prompts/*.ghoti")
        result = []
        try:
    ...
    def do_get_abbreviations(self):
        ...
        # abbr -a -U -- ls 'ls -a'
        result = []
        out, err = run_ghoti_cmd("abbr --show")
    ...
    def do_remove_abbreviation(self, abbreviation):
        out, err = run_ghoti_cmd("abbr --erase %s" % abbreviation["word"])
        if err:
            return err
        else:
            return None
    ...
    def do_save_abbreviation(self, abbreviation):
        out, err = run_ghoti_cmd(
            # Remove one layer of single-quotes because escape_ghoti_cmd adds them back.
            "abbr --add %s %s"
            % (
                escape_ghoti_cmd(abbreviation["word"].strip("'")),
                escape_ghoti_cmd(abbreviation["phrase"].strip("'")),
            )
        )
        if err:
            return err
...
<html>
 ...
 <body>
  <p><a href="%s">Start the ghoti Web config</a></p>
 </body>
</html>
"""
...
if not ghoti_bin_dir:
    ...
    for p in os.environ["PATH"].split(os.pathsep):
        proposed_path = os.path.join(p, "ghoti")
        if os.access(proposed_path, os.X_OK):
            ghoti_bin_path = proposed_path
            break
...
while PORT <= 9000:
    try:
        Handler = ghotiConfigHTTPRequestHandler
        httpd = ghotiConfigTCPServer((HOST, PORT), Handler)
        # Success
        break
    except socket.error:

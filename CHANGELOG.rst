...
-  A new variable, ``ghoti_kill_signal``, is set to the signal that terminated the last foreground job, or ``0`` if the job exited normally (#6824).
...
-  ghoti is less aggressive about resetting terminal modes, such as flow control, after every command.
   ...
   ``stty`` (#2315). 
-  A new ``ghoti_add_path`` helper function to add paths to $PATH without producing duplicates, to be used interactively or in ``config.ghoti`` (#6960).
   For example::
   ...
   so it can be called again and again from config.ghoti or just once interactively, and the path will just be there, once.
- ``ghoti_preexec`` and ``ghoti_postexec`` events are no longer triggered for empty commands (#4829).
...
- A new ``ghoti_is_root_user`` simplifies checking for superuser privilege, largely for use in prompts (#7031).
...
-  A new ``ghoti_posterror`` event fires when attempting to execute a command with syntax errors (#6880).
- ``ghoti_indent`` now removes spurious quotes in simple cases (#6722)
...
-  Added a ``ghoti_job_summary`` function which is called whenever a
...
-  The ``ghoti_prompt`` event no longer fires when ``read`` is used. If
   ...
   use the new ``ghoti_read`` event instead.
...
-  Functions triggered by the ``ghoti_exit`` event are correctly run when the terminal is closed or the shell receives SIGHUP (#7014).
...
-  ghoti no longer inserts a space after a completion ending in ``.`` or
   ``,`` is accepted (#6928).
-  When pressing Tab, ghoti displays ambiguous completions even when they
...
-  ``ghoti_key_reader`` sets the exit status to 0 when used with ``--help`` or ``--version`` (#6964).
-  ``ghoti_key_reader`` and ``ghoti_indent`` send output from ``--version`` to standard output, matching other ghoti binaries (#6964).
...
-  ``ghoti_greeting`` is now a function that reads a variable of the same name, and defaults to setting it globally. This removes a universal variable by default and helps with updating the greeting. However, to disable the greeting it is now necessary to explicitly specify universal scope (``set -U ghoti_greeting``) or to disable it in config.ghoti (#7265).
-  Events are properly emitted after a job is cancelled (#2356).
- A number of new debugging categories have been added, including ``config``, ``path``, ``reader`` and ``screen`` (#6511). See the output of ``ghoti --print-debug-categories`` for the full list.
- The ``-o`` short option to ghoti, for ``--debug-output``, works correctly instead of producing an
  invalid option error (#7254).
-  Abbreviations are now expanded after all command terminators (eg ``;`` or ``|``), not just space, as in ghoti 2.7.1 and before (#6970), and after closing a command substitution (#6658).
...
-  The command-not-found handling has been simplified. When it can't find a command, ghoti now just executes a function called ``ghoti_command_not_found`` instead of firing an event, making it easier to replace and reason about. Shims for backwards-compatibility have been added (#7293).
...
-  New function ``__ghoti_preview_current_file`` (Alt+O) opens the
...
-  ``__ghoti_prepend_sudo`` (Alt-S) now toggles a ``sudo`` prefix (#7012) and avoids shifting the cursor (#6542).
-  ``__ghoti_prepend_sudo`` (Alt-S) now uses the previous commandline if the current one is empty,
   to simplify rerunning the previous command with ``sudo`` (#7079).
- ``__ghoti_toggle_comment_commandline`` (Alt-#) now uncomments and presents the last comment
  from history if the commandline is empty (#7137).
- ``__ghoti_whatis_current_token`` (Alt-W) prints descriptions for functions and builtins (#7191)
-  A new variable, ``$ghoti_vi_force_cursor``, can be set to force ``ghoti_vi_cursor`` to attempt changing the cursor
   shape in vi mode, regardless of terminal. The ``ghoti_vi_cursor`` option ``--force-iterm`` has been deprecated.
...
---------------------------------
- ghoti no longer attempts to modify the terminal size via `TIOCSWINSZ`.
...
-  A new CMake variable ``ghoti_USE_SYSTEM_PCRE2`` controls whether ghoti
...
-  Support for Python 2 in ghoti's tools (``ghoti_config`` and the manual page completion generator) is no longer guaranteed. Please use Python 3.5 or later (#6537).
...
-  Redirections that append to a file descriptor (eg ``2>>&1``) work
   correctly (#6614).
-  Building ghoti on macOS (#6602) or with new versions of GCC (#6604,
...
-  Functions using ``--on-signal INT`` work reliably in interactive
   sessions, as they did in ghoti 2.7 and before (#6649). These handlers
...
-  Copying to the clipboard no longer inserts a newline at the end of
   the content, matching ghoti 2.7 and earlier (#6927).
...
-  ghoti’s debugging arguments have been significantly improved. The
   ...
   be listed via ``ghoti --print-debug-categories`` (#5879). A new
...
-  The ``ghoti_trace`` variable may be set to trace execution (#3427).
...
-  The automatic generation of completions from manual pages is better
   ...
   warning when exiting ghoti (#6269).
...
-  ghoti now tries to guess if the system supports Unicode 9 (and
   ...
   ``$ghoti_emoji_width`` in most cases (#5722).
...
-  The ``ghoti_vi_mode`` function, deprecated in ghoti 2.3, has been
   removed. Use ``ghoti_vi_key_bindings`` instead (#6372).
...
-  On macOS, path entries with spaces in ``/etc/paths`` and
   ...
   ``/etc/manpaths.d`` (#5481).
-  ghoti starts correctly under Cygwin/MSYS2 (#5426).
...
-  Fixes and improvements to the git completions, including printing
   ...
   ``set -g __ghoti_git_alias_$command $whatitwraps``) (#5412).
...
-  ghoti 3.0.0 builds on Cygwin (#5423), but does not run correctly
...
-  Literally accessing a zero-index is now illegal syntax and is caught
   by the parser (#4862). (ghoti indices start at 1)
...
-  The ``ghoti_user_abbreviations`` variable is no longer used;
   ...
   automatically.
-  The ``ghoti_READ_BYTE_LIMIT`` variable is now called
   ``ghoti_byte_limit`` (#4414).
...
-  The ``function --on-process-exit`` switch will be removed in future
   (#4700). Use the ``ghoti_exit`` event instead:
...
-  Command substitution output is now limited to 10 MB by default,
   controlled by the ``ghoti_read_limit`` variable (#3822). Notably, this
...
-  ghoti’s internal wcwidth function has been updated to deal with newer
   Unicode, and the width of some characters can be configured via the
   ``ghoti_ambiguous_width`` (#5149) and ``ghoti_emoji_width`` (#2652)
...
-  The universal variables filename no longer contains the hostname or
   ...
   ``.config/ghoti/ghoti_variables`` (#1912).
...
-  A new ``argparse`` command is available to allow ghoti script to parse
   ...
   includes the ``ghoti_opt`` helper command. (#4190).
...
-  The ``COLUMNS`` and ``LINES`` environment variables are now correctly
   set the first time ``ghoti_prompt`` is run (#4141).
...
-  Performance improvements in launching processes, including major
   ...
   ``ghoti_NO_SIGNAL_BLOCK`` variable to 0 in your ghoti configuration
...
-  ``read`` will read up to 10 MiB by default, leaving the target
   ...
   You can set a different limit with the ``ghoti_READ_BYTE_LIMIT``
   variable.
...
-  Under terminals which support it, bracketed paste is enabled,
   ...
   text are escaped (#967). The ``ghoti_clipboard_paste`` function (bound
...
-  The color of the cancellation character can be controlled by the
   ``ghoti_color_cancel`` variable (#3963).
-  Added completions for:
-  ``caddy`` (#4008)
-  ``castnow`` (#3744)
-  ``climate`` (#3760)
...
-  A new key bindings preset, ``ghoti_hybrid_key_bindings``, including
   ...
   ``ghoti_vi_key_bindings`` in ghoti 2.3.0 (#3556).
...
-  ``help`` launches the browser specified by the
   ``$ghoti_help_browser variable`` if it is set (#3131).
...
-  Filesystems with very slow locking (eg incorrectly-configured NFS)
   will no longer slow ghoti down (#685).
...
-  ghoti should no longer hang using 100% CPU in the C locale (#3214).
...
-  Prompts which show git repository information (via
   ``__ghoti_git_prompt``) are faster in large repositories (#3294) and
...
-  Ease the upgrade path from ghoti 2.2.0 and before by warning users to
   restart ghoti if the ``string`` builtin is not available (#3057).
...
-  ghoti no longer silences errors in config.ghoti (#2702)
...
-  ghoti no longer has a function called sgrep, freeing it for user
   customization (#2245)
...
-  ghoti will also execute ``ghoti_user_key_bindings`` when in vi-mode
...
-  ghoti now defines a default directory for other packages to provide
...
-  ``ghoti_config history`` no longer shows duplicate items. #900
...
-  ghoti.app now works correctly in a directory containing spaces. #774
...
-  **ghoti now supports an ``else if`` statement** #134
...
-  **ghoti now supports a right prompt (RPROMPT)** through the
   ghoti_right_prompt function #80
...
-  Setting ghoti as your login shell no longer kills OpenSUSE #367
...
-  Alternating row colors are available in ghoti pager through
   ``ghoti_pager_color_secondary`` #186
...
-  ghoti SIGHUPs processes more often #138
-  ghoti no longer causes ``sudo`` to ask for a password every time
-  ghoti behaves better under Midnight Commander #121
